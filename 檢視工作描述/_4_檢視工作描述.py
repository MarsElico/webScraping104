...
if __name__ == '__main__':
    ...
    # 根據出現次數排序
    unique_skills = 計算個別工作有幾成有什麼屬性(df, "business_trip")
    ...
    # 無需出差外派,需外派 的比例
    dictionary = 合併_無需出差外派_需外派()
    df = pandas.DataFrame({"col": dictionary.keys(), "rate": dictionary.values()})

    # 開始畫圖
    competitors_sum = int(sorted_skills_df["rate"].sum())
    length = len(sorted_skills_df)
    pie_template(sorted_skills_df, f"{competitors_sum}個工作分佈", 0, length, dpi=750,
